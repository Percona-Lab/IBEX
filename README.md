...
A [Model Context Protocol](https://modelcontextprotocol.io/) server that connects AI assistants to your workplace tools — Slack, Notion, Jira, ServiceNow, Salesforce, persistent memory ([PACK](https://github.com/Percona-Lab/PACK)), and Percona documentation search ([Percona-DK](https://github.com/Percona-Lab/percona-dk)).

Designed to run alongside [Open WebUI](https://github.com/open-webui/open-webui) with Percona's internal LLM servers for a self-hosted AI assistant with access to your internal tools. Branded as **IBEX** in the UI.

## Install

One command. No prerequisites — the installer handles everything (Node.js, Python, Git, Open WebUI, MCPO).
...
4. Clones the IBEX repository to `~/IBEX` (pinned to the stable release tag)
5. Walks you through connector credentials (Slack, Notion, Jira, etc.)
6. Installs **Open WebUI** in a virtual environment (via uv)
7. Installs **[MCPO](https://github.com/open-webui/mcpo)** — MCP-to-OpenAPI proxy for tool integration
8. Sets up **[Percona-DK](https://github.com/Percona-Lab/percona-dk)** — semantic Percona documentation search
9. Optionally sets up **https://ibex** as a local domain (mkcert + Caddy)
10. Starts all services, creates your account, and opens the browser — already logged in
...
The installer defaults to the latest **stable tag** (currently `v1.0`). To install a development version:

```bash
IBEX_VERSION=dev curl -fsSL https://raw.githubusercontent.com/Percona-Lab/IBEX/main/install-ibex | bash
```

| Branch/Tag | Purpose |
|------------|---------|
| `v1.0` tag | Frozen stable release (installer default) |
| `main` | Installer bootstrap only — pinned to stable tag |
...
| **Percona-DK** | stdio | `search_percona_docs`, `get_percona_doc` | Semantic search across Percona documentation |
...
```
Browser → https://ibex (Caddy) → Open WebUI (:8080) → MCPO (:8010) → MCP servers (:3001-3007) → APIs
                                       ↓                    ↓
                                  Percona LLM servers   percona-dk (stdio) → ChromaDB
```

- **Open WebUI** runs natively (no Docker) via a Python virtual environment managed by `uv`
- **MCPO** proxies MCP servers as OpenAPI endpoints for Open WebUI tool integration
- **MCP servers** run as supervised Node.js processes with auto-restart
- **Percona-DK** runs as a stdio MCP server, proxied by MCPO (no dedicated port)
- **Process supervisor** (`start-ibex.cjs`) monitors all processes, restarts on crash
...
- **PID file**: `~/.ibex-logs/supervisor.pid`
...
```
curl | bash
  → install-ibex (bash)         Installs Node.js + Git, fetches install-node.cjs from pinned tag
  → install-node.cjs (node)     Installs uv, clones repo, credentials, Open WebUI, MCPO, Percona-DK
  → start-ibex.cjs              Process supervisor — starts all services, applies branding
  → configure-owui.cjs          Creates account, sets system prompt, configures tool servers and models
```
...
```bash
git clone --branch v1.0 https://github.com/Percona-Lab/IBEX.git ~/IBEX
cd ~/IBEX
npm install
```
...
├── start-ibex.cjs             # Process supervisor (starts/monitors all services)
├── scripts/
│   ├── configure-owui.cjs     # Auto-configures Open WebUI (account, prompt, tool servers, models)
...
├── branding/                  # IBEX logo and icon assets
└── package.json
```
